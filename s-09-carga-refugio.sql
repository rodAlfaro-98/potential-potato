...
--@Descripción:     Script de creación de la carga centro operativo del proyecto 
--de Pet Home
...
declare
  ...
  v_maxima number(5,0);
begin
  nombres := v_nombre('Garantizamos','Lo mas importante es','Nos enfocamos en',
    'Nuestra prioridad es');
  app := v_apellido_paterno('la satisfaccion','el cuidado','la felicidad',
    'tus animalitos');
  ...
  for i in 1 .. 100 loop
    if(mod(i,4) = 0 or mod(i,4) = 3)then
        ...
        v_numero_registro := 'RF'||abs(mod(dbms_random.random,99)+1)||'PT'
          ||abs(mod(dbms_random.random,99)+1);
