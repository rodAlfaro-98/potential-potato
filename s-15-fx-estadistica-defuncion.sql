...
begin
	if(año = '') then
		insert into mascota_defuncion_temp 
        (mascota_id,nombre,fecha_status,causa_muerte,cliente_id)
        select mascota_id, nombre, fecha_status, causa_muerte, 
        nvl(cliente_id,0)
        from mascota
        where lower(causa_muerte) like '%'||lower(causa)||'%';
    else 
		insert into mascota_defuncion_temp (mascota_id,nombre,
        fecha_status,causa_muerte,cliente_id)
        select mascota_id, nombre, fecha_status, causa_muerte, 
        nvl(cliente_id,0)
        from mascota
        where lower(causa_muerte) like '%'||lower(causa)||'%'
        and fecha_status between to_date(año,'yyyy') and 
        to_date(año,'yyyy')+365;
    end if;
